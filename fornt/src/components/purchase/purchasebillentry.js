...
// Mirrors the server's GST math (back/controllers/purchasecontroller.js) for a live preview.
...
const PurchaseBillEntry = () => {
  ...
  useEffect(() => {
    fetchJson('/api/suppliers').then(setSupplierList).catch(() => setSupplierList([]));
  ...
  const loadBills = useCallback(async () => {
    ...
    try {
      const data = await fetchJson(`/api/purchases${params.toString() ? `?${params}` : ''}`);
      setBills(data);
      setBillsStatus(data.length ? '' : 'No purchase bills for this range.');
    } catch (error) {
      setBills([]);
      setBillsStatus(error.message || 'Unable to load purchase bills.');
    }
  }, [billFilter.startDate, billFilter.endDate, billFilter.q]);
  useEffect(() => { loadBills(); }, [loadBills]);

  const supplierExists = (name) => !name || supplierList.some((s) => s.name?.toLowerCase() === name.trim().toLowerCase());
  ...
  const handleSupplierNameChange = (value) => {
    ...
    if (supplierWarning) setSupplierWarning('');
  };
  const handleSupplierBlur = (value) => {
    const known = supplierExists(value);
    setSupplierWarning(known ? '' : 'This supplier is not in the list. Please add the supplier first.');
    if (!known) return;
    const match = supplierList.find((s) => s.name?.toLowerCase() === value.trim().toLowerCase());
    if (!match) return;
    setSupplier((s) => ({ ...s, name: match.name, supplierId: match._id || '', gstin: match.gstin || '', state: match.state || '' }));
    setBillMeta((m) => ({ ...m, placeOfSupply: match.state || m.placeOfSupply }));
  };
  ...
  const handleSave = async (event) => {
    ...
    if (!supplierExists(supplier.name)) {
      setSupplierWarning('This supplier is not in the list. Please add the supplier first.');
      return;
    }
    if (!lines.length) {
    ...
    try {
      ...
      setSaveStatus({ type: 'error', text: error.message || 'Unable to save purchase bill.' });
    }
  };
    ...
    <div className="gst-bill-layout">
          ...
          <fieldset>
            ...
            <div className="gst-bill-header-row">
              <div className="gst-input-field">
                ...
                />
                {supplierWarning && <p className="gst-field-warning" role="alert">{supplierWarning}</p>}
              </div>
              <datalist id="purchase-supplier-list">{supplierList.map((s) => <option key={s._id} value={s.name} />)}</datalist>
          ...
          <fieldset>
            ...
            <div className="gst-line-entry">
              <div className="gst-input-field">
                ...
                <input type="text" className="gst-text-input textbox-middle" list="purchase-product-list" ref={productNameRef} value={currentLine.name}
                  onChange={(e) => handleProductNameChange(e.target.value)}
                  onKeyDown={(e) => focusNextOnEnter(e, hsnRef)} />
              </div>
              <datalist id="purchase-product-list">{productList.map((p) => <option key={p._id} value={p.name} />)}</datalist>
              <div className="gst-input-field">
                <label>HSN</label><br />
          ...
          <fieldset>
            ...
            <div className="gst-totals-row">
              ...
              <div className="gst-totals-side">
                <div className="gst-input-field"><label>Cash paid</label><input type="number" className="gst-small-input" min="0" step="0.01" value={billMeta.cash} onChange={(e) => setBillMeta((m) => ({ ...m, cash: e.target.value }))} /></div>
      ...
      <section className="gst-bill-list-card">
        ...
        <div className="gst-bill-filter" role="search">
          ...
          <label><span>Search</span><input type="search" className='bill-filter-option' value={billFilter.q} onChange={(e) => setBillFilter((f) => ({ ...f, q: e.target.value }))} placeholder="Bill no. or supplier" /></label>
        </div>
        <div className="gst-table-wrapper">
          <table className="gst-table">
            <thead><tr><th>Bill No.</th><th>Supplier</th><th>Grand total</th><th>Actions</th></tr></thead>
            <tbody>
