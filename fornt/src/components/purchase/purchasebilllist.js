...
// Dedicated report page for purchase bills: free-text search, date range and
// tax-type filters, server-side pagination, plus a read-only view modal.
const PurchaseBillList = () => {
  ...
  const loadBills = useCallback(async (activeFilters = filters, activePage = page) => {
    ...
    try {
      ...
      setBills([]);
      setMessage(error.message || 'Unable to load purchase bills.');
    } finally {
      setLoading(false);
    }
  }, [filters, page]);
    ...
    <main className="gst-bill-page">
      <section className="gst-bill-card gst-bill-list-page-card">
        <header className="gst-bill-list-header">
          <div>
            <p className="gst-bill-list-eyebrow">Purchase report</p>
            <h1>Purchase Bills</h1>
            <p>Search every purchase bill by supplier or bill number, filter by date and tax type.</p>
          </div>
        ...
        <form className="gst-bill-filter" onSubmit={submitFilter}>
          <label><span>Search</span>
            <input type="search" className="bill-filter-option" value={filters.q} onChange={(e) => update('q', e.target.value)} placeholder="Supplier or bill no." />
          </label>
        ...
        <div className="gst-table-wrapper">
          <table className="gst-table">
            <thead>
              <tr><th>Bill No.</th><th>Date</th><th>Supplier</th><th>Tax type</th><th>Grand total</th><th>Cash</th><th>Credit</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan="8" className="gst-table-state">Loading purchase bills…</td></tr>
              ) : bills.length === 0 ? (
