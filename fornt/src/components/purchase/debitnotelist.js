...
// Dedicated report page for debit notes: free-text search (note no., original bill
// no. or supplier), date range, server-side pagination, plus a read-only view modal.
const DebitNoteList = () => {
  ...
  const loadNotes = useCallback(async (activeFilters = filters, activePage = page) => {
    ...
    try {
      const result = await fetchJson(`/api/debit-notes?${params.toString()}`);
      setNotes(result.data || []);
      setTotal(result.total || 0);
      setPages(result.pages || 1);
    } catch (error) {
      setNotes([]);
      setMessage(error.message || 'Unable to load debit notes.');
    } finally {
      setLoading(false);
    }
  }, [filters, page]);
    ...
    <main className="gst-bill-page">
      <section className="gst-bill-card gst-bill-list-page-card">
        <header className="gst-bill-list-header">
          <div>
            <p className="gst-bill-list-eyebrow">Debit note report</p>
            <h1>Debit Notes</h1>
            <p>Search every debit note by note number, original bill number, or supplier.</p>
          </div>
        ...
        <form className="gst-bill-filter" onSubmit={submitFilter}>
          <label><span>Search</span>
            <input type="search" className="bill-filter-option" value={filters.q} onChange={(e) => update('q', e.target.value)} placeholder="Note no., bill no. or supplier" />
          </label>
        ...
        <div className="gst-table-wrapper">
          <table className="gst-table">
            <thead>
              <tr><th>Note No.</th><th>Date</th><th>Against bill</th><th>Supplier</th><th>Reason</th><th>Amount</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan="7" className="gst-table-state">Loading debit notes…</td></tr>
              ) : notes.length === 0 ? (
