...
const CreditNoteEntry = () => {
  ...
  const loadNotes = useCallback(async () => {
    ...
    try {
      const data = await fetchJson(`/api/credit-notes${params.toString() ? `?${params}` : ''}`);
      setNotes(data);
      setNotesStatus(data.length ? '' : 'No credit notes for this range.');
    } catch (error) {
      setNotes([]);
      setNotesStatus(error.message || 'Unable to load credit notes.');
    }
  ...
  const loadNoteForEdit = (note) => {
    ...
    setCustomer({
      ...
      customerId: note.customer?.customerId || '',
  ...
  const handleSave = async (event) => {
    ...
    if (!lines.length) {
      setSaveStatus({ type: 'error', text: 'Add at least one product before saving the credit note.' });
      return;
    }
    const payload = {
    ...
    try {
      ...
      setSaveStatus({ type: 'error', text: error.message || 'Unable to save credit note.' });
    }
  };
  ...
  const deleteNote = async (note) => {
    ...
    try {
      ...
      setNotesStatus(error.message || 'Unable to delete credit note.');
    }
  };
    ...
    <div className="gst-bill-layout">
        ...
        <form className="gst-bill-form" onSubmit={handleSave} noValidate>
          <fieldset>
            ...
            <div className="gst-bill-header-row">
              ...
              <div className="gst-input-field gst-narrow"><label>Credit Note No:</label><br /><input type="text" className="gst-text-input gst-compact-input" placeholder="Auto" value={billMeta.creditNoteNumber} onChange={(e) => setBillMeta((m) => ({ ...m, creditNoteNumber: e.target.value }))} disabled={Boolean(editingId)} /></div>
          ...
          <fieldset>
            ...
            <div className="gst-bill-header-row">
              <div className="gst-input-field"><label>Customer Name:</label><br /><input type="text" className="gst-text-input" value={customer.name} readOnly /></div>
              <div className="gst-input-field textbox-middle"><label>GSTIN:</label><br /><input type="text" className="gst-text-input gst-compact-input" value={customer.gstin} readOnly /></div>
              <div className="gst-input-field">
          ...
          <fieldset>
            ...
            <div className="gst-line-entry">
              <div className="gst-input-field">
                ...
                <input type="text" className="gst-text-input textbox-middle" list="credit-note-product-list" ref={productNameRef} value={currentLine.name}
                  onChange={(e) => handleProductNameChange(e.target.value)}
                  onKeyDown={(e) => focusNextOnEnter(e, hsnRef)} />
              </div>
              <datalist id="credit-note-product-list">{productList.map((p) => <option key={p._id} value={p.name} />)}</datalist>
              <div className="gst-input-field">
                <label>HSN</label><br />
          ...
          <fieldset>
            ...
            <div className="gst-totals-row">
              <table className="gst-table gst-totals-table">
                <thead>
                  <tr><th>Subtotal</th><th>CGST</th><th>SGST</th><th>IGST</th><th>Round off</th><th>Credit note amount</th></tr>
                </thead>
                <tbody>
                  <tr>
      ...
      <section className="gst-bill-list-card">
        ...
        <div className="gst-bill-filter" role="search">
          ...
          <label><span>Search</span><input type="search" className='bill-filter-option' value={noteFilter.q} onChange={(e) => setNoteFilter((f) => ({ ...f, q: e.target.value }))} placeholder="Note no., bill no. or customer" /></label>
        </div>
        <div className="gst-table-wrapper">
          <table className="gst-table">
            <thead><tr><th>Note No.</th><th>Against</th><th>Customer</th><th>Amount</th><th>Actions</th></tr></thead>
            <tbody>
...
export default CreditNoteEntry;
